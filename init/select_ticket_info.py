        ...
        while 1:
            try:
                ...
                if queryResult.get("status"):
                    ...
                    print(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} 正在第{num}次查询 停留时间：{random_time} 乘车日期: {','.join(TickerConfig.STATION_DATES)} 乘车人: {','.join(TickerConfig.TICKET_PEOPLES)} 车次：{','.join(TickerConfig.STATION_TRAINS) or '所有车次'} 下单无票{nateMsg} 耗时：{(datetime.datetime.now() - now).microseconds / 1000} {queryResult.get('cdn')}")
